...
const MOCK_DEMO_DISPLAY_NAME = "Demo User";
...
const mockDemoManifestPath = path.join(
  dataDir,
  "mock_demo_cache_manifest.json",
);
...
function writeSpotifyCacheDataRecord(
  userId,
  cacheKey,
  data,
...
function isDemoSession(cookies) {
  return (
    cookies?.sp_auth_mode === MOCK_DEMO_AUTH_MODE &&
    isDemoUserId(cookies?.sp_user_id)
  );
}
...
function setDemoSessionCookies(res) {
  setCookie(res, "sp_auth_mode", MOCK_DEMO_AUTH_MODE, {
    path: "/",
    maxAge: 30 * 24 * 60 * 60,
  });
  setCookie(res, "sp_scope", MOCK_DEMO_SCOPE, {
    path: "/",
    maxAge: 30 * 24 * 60 * 60,
  });
  setCookie(res, "sp_user_id", MOCK_DEMO_CACHE_USER_ID, {
    path: "/",
    maxAge: 30 * 24 * 60 * 60,
  });
...
function readMockDemoManifest() {
  ...
  return {
    ...
    seedThrottleUntil:
      typeof parsed.seedThrottleUntil === "string"
        ? parsed.seedThrottleUntil
        : null,
    seedThrottleReason:
      typeof parsed.seedThrottleReason === "string"
        ? parsed.seedThrottleReason
        : null,
    resources:
...
function updateMockDemoManifestResource({
  kind,
  id,
  status,
  cacheKey = null,
  ...
  manifest.resources[resourceKey] = {
    ...
    status,
    cacheKey:
      typeof cacheKey === "string" ? cacheKey : (existing?.cacheKey ?? null),
    note: typeof note === "string" ? note : (existing?.note ?? null),
    firstObservedAt:
      typeof existing?.firstObservedAt === "string"
        ? existing.firstObservedAt
        : now,
    lastObservedAt: now,
    observations:
      Number(existing?.observations) > 0
        ? Number(existing.observations) + 1
        : 1,
...
function setMockDemoSeedThrottle({ retryAfterSeconds, reason }) {
  ...
  manifest.seedThrottleReason =
    typeof reason === "string" ? reason : "spotify_retry_after";
  writeJsonFileAtomic(mockDemoManifestPath, manifest);
}
...
function getStaticContentType(filePath) {
  return (
    staticContentTypes.get(path.extname(filePath).toLowerCase()) ||
...
function syncMockDemoManifestSeedState() {
  ...
  const playlistIndexRecord = readSpotifyCacheRecord(
  ...
  updateMockDemoManifestResource({
    ...
    extra: playlistIndexRecord
      ? {
          total: Number(playlistIndexRecord?.data?.total) || 0,
          cachedAt: playlistIndexRecord.fetchedAt,
        }
      : null,
  });
  ...
  for (const playlistId of MOCK_DEMO_PLAYLIST_IDS) {
    ...
    const itemCount = Array.isArray(record?.data?.items)
      ? record.data.items.length
      : 0;
    updateMockDemoManifestResource({
...
function sendDemoCachePayload(
  res,
  { cacheKey, kind, id, missingError, missingMessage, transform },
) {
  const record = readSpotifyCacheRecord(MOCK_DEMO_CACHE_USER_ID, cacheKey);
  if (!record) {
  ...
  updateMockDemoManifestResource({
    ...
    extra: {
      ...
      itemCount: Array.isArray(record?.data?.items)
        ? record.data.items.length
        : null,
    },
  });
  ...
  sendJson(
    res,
    200,
    typeof transform === "function" ? transform(record) : record.data,
  );
  return true;
}

function shouldAttemptSpotifyRefresh({
  record,
  nowMs,
  ...
  const stale = Number.isFinite(fetchedAtMs)
    ? nowMs - fetchedAtMs >= refreshWindowMs
    : true;
...
function normalizePlaylistTracks(items) {
  ...
  for (const row of items) {
    ...
    const album =
      typeof entry.album?.name === "string" ? entry.album.name : null;
...
function computeTopArtistsFromPlaylist(
  tracks,
  { maxSongsPerArtist = 5, maxArtists = Number.POSITIVE_INFINITY } = {},
) {
  ...
  for (const track of tracks) {
    ...
    const artistsDetailed = Array.isArray(track?.artistsDetailed)
      ? track.artistsDetailed
      : [];
    ...
    for (const artistName of artistNames) {
      ...
      if (!existing.artistId && artistsDetailed.length) {
        const match = artistsDetailed.find(
          a => a?.name === artistName && typeof a?.id === "string" && a.id,
        );
  ...
  const scored = Array.from(byArtist.values()).map(artist => {
    ...
    const avgRank = n
      ? topTracks.reduce((sum, t) => sum + t.rank, 0) / n
      : null;
  ...
  return scored
    .slice(0, maxArtists)
    .map((artist, idx) => ({ ...artist, rank: idx + 1 }));
}
...
function shouldRefreshArtistImageRecord(record, nowMs) {
  ...
  return (
    ...
    (!cachedImageUrl &&
      Number.isFinite(fetchedAtMs) &&
      nowMs - fetchedAtMs >= ARTIST_IMAGE_NULL_RETRY_MS)
  );
}
...
function hasCompleteAlbumTracks(recordOrPayload, expectedTotal = null) {
  const payload =
    recordOrPayload?.data && typeof recordOrPayload.data === "object"
      ? recordOrPayload.data
      : recordOrPayload;
  const items = Array.isArray(payload?.items) ? payload.items : [];
  if (!items.length)
    return Number(payload?.total) === 0 || Number(expectedTotal) === 0;
...
function buildArtistProfilePayload(data) {
  ...
  return {
    ...
    externalUrl:
      typeof data?.external_urls?.spotify === "string"
        ? data.external_urls.spotify
        : null,
    genres: Array.isArray(data?.genres)
      ? data.genres.filter(genre => typeof genre === "string" && genre)
      : [],
    popularity: Number.isFinite(data?.popularity) ? data.popularity : null,
    followersTotal: Number.isFinite(data?.followers?.total)
...
async function primeArtistCachesForArtistIds({
  cacheUserId,
  artistIds,
  accessToken,
  debugContext,
  ...
  const idsToRefresh = artistIds.filter(
    artistId =>
      shouldRefreshArtistProfileRecord(
        readSpotifyCacheRecord(cacheUserId, `artist_${artistId}_profile`),
        nowMs,
      ) ||
      shouldRefreshArtistImageRecord(
        readSpotifyCacheRecord(cacheUserId, `artist_${artistId}_image`),
        nowMs,
      ),
  );
...
async function primeAlbumCachesForAlbumRefs({
  sourceUserId,
  cacheUserId,
  albumRefs,
  accessToken,
  ...
  if (!Array.isArray(albumRefs) || albumRefs.length === 0)
    return { paused: false };
  ...
  for (const album of albumRefs) {
    ...
    const expectedTotal = Number.isFinite(album?.totalTracks)
      ? album.totalTracks
      : null;
    ...
    let albumTracksPayload = hasCompleteAlbumTracks(
      existingDemoRecord,
      expectedTotal,
    )
    ...
    if (!hasCompleteAlbumTracks(albumTracksPayload, expectedTotal)) {
      ...
      if (
        Number.isFinite(fetchedAlbum?.retryAfterSeconds) &&
        fetchedAlbum.retryAfterSeconds > 0
      ) {
        if (cacheUserId === MOCK_DEMO_CACHE_USER_ID) {
          setMockDemoSeedThrottle({
      ...
      if (
        !fetchedAlbum.ok ||
        !hasCompleteAlbumTracks(fetchedAlbum.data, expectedTotal)
      ) {
        if (cacheUserId === MOCK_DEMO_CACHE_USER_ID) {
          updateMockDemoManifestResource({
            kind: "album_tracks",
            ...
            extra: {
              ...
              expectedTotal,
              retryAfterSeconds:
                Number(fetchedAlbum?.retryAfterSeconds) || null,
    ...
    if (cacheUserId === MOCK_DEMO_CACHE_USER_ID) {
      updateMockDemoManifestResource({
        ...
        extra: {
          ...
          itemCount: Array.isArray(albumTracksPayload?.items)
            ? albumTracksPayload.items.length
            : 0,
...
async function primeArtistImageCacheForPlaylist({
  cacheUserId,
  playlistPayload,
  accessToken,
  debugContext,
...
function hasPlaylistItems(record) {
  ...
  const total =
    Number(record?.data?.total) || Number(record?.data?.tracks?.total) || 0;
  return total === 0;
}
...
function playlistPageContainsAllIds(payload, playlistIds) {
  const items = Array.isArray(payload?.items) ? payload.items : [];
  if (!items.length || !Array.isArray(playlistIds) || !playlistIds.length)
    return false;
...
function filterPlaylistPageByIds(payload, allowedIds) {
  ...
  const items = Array.isArray(payload?.items)
    ? payload.items.filter(item => allowedIds.has(item?.id))
    : [];
  return {
...
async function fetchPlaylistTracksAll({
  ...
  const firstTracks =
    first.data?.tracks && typeof first.data.tracks === "object"
      ? first.data.tracks
      : first.data;
  const items = Array.isArray(firstTracks?.items)
    ? firstTracks.items.slice()
    : [];
  ...
  while (nextOffset < total) {
    ...
    const pageTracks =
      page.data?.tracks && typeof page.data.tracks === "object"
        ? page.data.tracks
        : page.data;
...
async function fetchCurrentUserPlaylistsAll({ accessToken, debugContext }) {
  ...
  const items = Array.isArray(first.data?.items)
    ? first.data.items.slice()
    : [];
...
async function seedMockDemoPlaylistCache({
  sourceUserId,
  accessToken,
  debugContext,
}) {
  ...
  let playlistPayload =
    readSpotifyCacheRecord(sourceUserId, "me_playlists_all")?.data ?? null;
  if (
    !hasUsablePlaylistPagePayload(playlistPayload) ||
    !playlistPageContainsAllIds(playlistPayload, MOCK_DEMO_PLAYLIST_IDS)
  ) {
    logDemoSeed("owner_playlists_fetch_start", { sourceUserId });
    const fetchedPlaylists = await fetchCurrentUserPlaylistsAll({
      accessToken,
      debugContext,
    });
    if (
      Number.isFinite(fetchedPlaylists?.retryAfterSeconds) &&
      fetchedPlaylists.retryAfterSeconds > 0
    ) {
      setMockDemoSeedThrottle({
    ...
    if (
      !fetchedPlaylists.ok ||
      !hasUsablePlaylistPagePayload(fetchedPlaylists.data)
    ) {
      logDemoSeed("owner_playlists_fetch_failed", {
        sourceUserId,
        status: fetchedPlaylists?.status || null,
        ok: Boolean(fetchedPlaylists?.ok),
        itemCount: Array.isArray(fetchedPlaylists?.data?.items)
          ? fetchedPlaylists.data.items.length
          : null,
    ...
    logDemoSeed("owner_playlists_fetch_cached", {
      sourceUserId,
      itemCount: Array.isArray(playlistPayload?.items)
        ? playlistPayload.items.length
        : null,
  ...
  const demoPlaylistsPayload = filterPlaylistPageByIds(
    playlistPayload,
    MOCK_DEMO_PLAYLIST_ID_SET,
  );
  if (
    !Array.isArray(demoPlaylistsPayload?.items) ||
    demoPlaylistsPayload.items.length === 0
  ) {
  ...
  logDemoSeed("demo_playlist_index_cached", {
    ...
    total:
      Number(demoPlaylistsPayload?.total) || demoPlaylistsPayload.items.length,
  });
  updateMockDemoManifestResource({
    kind: "playlists_index",
    id: "me",
    ...
    extra: {
      total:
        Number(demoPlaylistsPayload?.total) ||
        (Array.isArray(demoPlaylistsPayload?.items)
          ? demoPlaylistsPayload.items.length
          : 0),
      cachedAt: storedAt,
    },
  });
  ...
  for (const playlistId of MOCK_DEMO_PLAYLIST_IDS) {
    ...
    const existingDemoTracksRecord = readSpotifyCacheRecord(
      MOCK_DEMO_CACHE_USER_ID,
      cacheKey,
    );
    const existingOwnerTracksRecord = readSpotifyCacheRecord(
      sourceUserId,
      cacheKey,
    );
    ...
    if (!hasPlaylistItems({ data: playlistTracksPayload })) {
      ...
      const fetchedTracks = await fetchPlaylistTracksAll({
        ...
        accessToken,
        debugContext: {
          ...(debugContext && typeof debugContext === "object"
            ? debugContext
            : {}),
          playlistId,
      ...
      if (
        Number.isFinite(fetchedTracks?.retryAfterSeconds) &&
        fetchedTracks.retryAfterSeconds > 0
      ) {
        setMockDemoSeedThrottle({
      ...
      if (
        !fetchedTracks.ok ||
        !hasPlaylistItems({ data: fetchedTracks.data })
      ) {
        logDemoSeed("playlist_tracks_fetch_failed", {
          ...
          itemCount: Array.isArray(fetchedTracks?.data?.items)
      ...
      logDemoSeed("playlist_tracks_fetched_and_cached", {
        ...
        playlistId,
        playlistName,
        itemCount: Array.isArray(playlistTracksPayload?.items)
          ? playlistTracksPayload.items.length
          : null,
      ...
      logDemoSeed("playlist_tracks_cache_hit", {
        ...
        playlistId,
        playlistName,
        source: hasPlaylistItems(existingDemoTracksRecord)
          ? "demo_cache"
          : "owner_cache",
        itemCount: Array.isArray(playlistTracksPayload?.items)
          ? playlistTracksPayload.items.length
...
async function fetchAlbumTracksAll({ albumId, accessToken, debugContext }) {
  ...
  return {
    ok: true,
    status: 200,
    data: { items, offset: 0, limit, total, albumId },
  };
}
...
function logSpotifyRequest({
  ...
  if (typeof context?.playlistId === "string" && context.playlistId)
    parts.push(`playlist=${context.playlistId}`);
  if (typeof context?.albumId === "string" && context.albumId)
    parts.push(`album=${context.albumId}`);
  if (typeof context?.artistId === "string" && context.artistId)
    parts.push(`artist=${context.artistId}`);
  if (typeof context?.phase === "string" && context.phase)
...
function redirectWithAuthError(res, code, status) {
  ...
  if (Number.isFinite(status))
    location.searchParams.set("auth_status", String(status));
  res.statusCode = 302;
...
async function spotifyFetch(
  url,
  { label, debugContext, method = "GET", ...options } = {},
) {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/public/preview") {
    (async () => {
      ...
      const cachedHasItems = cachedRecord
        ? hasPlaylistItems(cachedRecord)
        : false;
        ...
        const topArtists = computeTopArtistsFromPlaylist(tracks, {
          ...
          const imageUrl =
            typeof artist?.artistId === "string" && artist.artistId
              ? (readSpotifyCacheRecord(
                  cacheUserId,
                  `artist_${artist.artistId}_image`,
                )?.data?.imageUrl ?? null)
              : null;
      ...
      if (cachedRecord) {
        ...
        res.setHeader(
          "x-sp-cache-last-refreshed-at",
          cachedRecord.lastRefreshedAt,
        );
      ...
      try {
        ...
        res.setHeader(
          "x-sp-cache",
          cachedRecord ? "refreshed" : "miss_refreshed",
        );
  ...
  if (req.method === "GET" && url.pathname === "/auth/login") {
    const clientId = process.env.SPOTIFY_CLIENT_ID;
    const redirectUri =
      process.env.SPOTIFY_REDIRECT_URI || "http://127.0.0.1:5173/auth/callback";
  ...
  if (req.method === "GET" && url.pathname === "/auth/callback") {
    ...
    (async () => {
      try {
        const token = await spotifyTokenExchange({
          code,
        ...
        if (typeof accessToken === "string") {
          setCookie(res, "sp_access", accessToken, {
            path: "/",
            maxAge: Math.max(0, (Number(expiresIn) || 3600) - 30),
          });
        }
        if (typeof refreshToken === "string") {
          setCookie(res, "sp_refresh", refreshToken, {
            path: "/",
            maxAge: 30 * 24 * 60 * 60,
          });
        }
        if (typeof scope === "string") {
          setCookie(res, "sp_scope", scope, {
            path: "/",
            maxAge: 30 * 24 * 60 * 60,
          });
        }
        setCookie(res, "sp_auth_mode", "spotify", {
          path: "/",
          maxAge: 30 * 24 * 60 * 60,
        });
        if (me?.data && typeof me.data === "object") {
          if (typeof me.data.id === "string")
            setCookie(res, "sp_user_id", me.data.id, {
              path: "/",
              maxAge: 30 * 24 * 60 * 60,
            });
          if (typeof me.data.display_name === "string")
            setCookie(res, "sp_user_name", me.data.display_name, {
              path: "/",
              maxAge: 30 * 24 * 60 * 60,
              httpOnly: false,
            });
        ...
        if (typeof accessToken === "string") {
          try {
            ...
            const cacheKey = `playlist_${PUBLIC_PREVIEW_PLAYLIST_ID}_tracks_all`;
            let previewPayload =
              readSpotifyCacheRecord(cacheUserId, cacheKey)?.data ?? null;
            if (
              !previewPayload ||
              !hasPlaylistItems({ data: previewPayload })
            ) {
            ...
            console.error(
              "[demo-seed] callback prime failed",
              JSON.stringify(
                {
  ...
  if (req.method === "GET" && url.pathname === "/api/session") {
    ...
    const hasAccess =
      typeof cookies.sp_access === "string" && cookies.sp_access.length > 0;
    const hasRefresh =
      typeof cookies.sp_refresh === "string" && cookies.sp_refresh.length > 0;
    const loggedIn = demoSession || hasAccess || hasRefresh;
    const scopes =
      typeof cookies.sp_scope === "string"
        ? cookies.sp_scope.split(" ").filter(Boolean)
        : [];
  ...
  if (req.method === "GET" && url.pathname === "/api/ranking") {
    const userId =
      typeof cookies.sp_user_id === "string" ? cookies.sp_user_id : null;
    const demoSession = isDemoSession(cookies);
    const hasAccess =
      typeof cookies.sp_access === "string" && cookies.sp_access.length > 0;
    const hasRefresh =
  ...
  if (req.method === "GET" && url.pathname === "/api/me/playlists") {
    ...
    (async () => {
      ...
      const nowMs = Date.now();
      let cachedRecord = canUseServerCache
        ? readSpotifyCacheRecord(cacheUserId, cacheKey)
        : null;
      if (cachedRecord && !hasUsablePlaylistPage(cachedRecord.data))
        cachedRecord = null;
      ...
      if (canUseServerCache && cachedRecord && !forceRefresh) {
        ...
        if (!decision.shouldRefresh) {
          res.setHeader(
            "x-sp-cache",
            decision.stale ? "hit_throttled" : "hit_fresh",
          );
          res.setHeader("x-sp-cache-fetched-at", cachedRecord.fetchedAt);
          res.setHeader(
            "x-sp-cache-last-refreshed-at",
      ...
      try {
        if (!all) {
          const result = await fetchWithRefresh(token =>
            callPlaylists(token, { offset }),
          );
          if (!result.ok) {
            sendJson(res, result.status, {
              error: "spotify_playlists_failed",
              details: result.data,
              retryAfterSeconds: result.retryAfterSeconds,
            });
            return;
        ...
        const first = await fetchWithRefresh(token =>
          callPlaylists(token, { offset: 0 }),
        );
        if (!first.ok) {
        ...
        const firstItems = Array.isArray(first.data?.items)
          ? first.data.items
          : null;
        ...
        if (total > 0 && items.length === 0) {
            ...
            JSON.stringify(
              {
                ...
                href:
                  typeof first.data?.href === "string" ? first.data.href : null,
                context: debugContext,
              },
              null,
              2,
          ...
          sendJson(res, 502, {
            ...
            message:
              "Spotify returned a playlist total but no playlist items on the first page.",
            details: {
              total,
              offset: Number(first.data?.offset) || 0,
              limit: Number(first.data?.limit) || limit,
        ...
        while (nextOffset < total) {
          ...
          const page = await fetchWithRefresh(token =>
            callPlaylists(token, { offset: nextOffset }),
          );
          if (!page.ok) {
            if (canUseServerCache && cachedRecord) {
              res.setHeader("x-sp-cache", "hit_fallback");
            ...
            sendJson(res, page.status, {
              error: "spotify_playlists_failed",
              details: page.data,
              retryAfterSeconds: page.retryAfterSeconds,
              partial: { items, total, nextOffset },
            });
            return;
          }

          const pageItems = Array.isArray(page.data?.items)
            ? page.data.items
            : [];
          if (nextOffset < total && pageItems.length === 0) {
              ...
              JSON.stringify(
                {
                  ...
                  limit,
                  href:
                    typeof page.data?.href === "string" ? page.data.href : null,
                  context: debugContext,
                },
                null,
                2,
            ...
            sendJson(res, 502, {
              error: "spotify_playlists_incomplete",
              message:
                "Spotify returned an empty playlist page before the reported total was reached.",
              details: {
                total,
                offset: nextOffset,
                limit,
  ...
  if (req.method === "GET" && url.pathname.startsWith("/api/tracks/")) {
    ...
    (async () => {
      ...
      const cacheKey = `track_${trackId}_artists`;
      const cachedRecord = cacheUserId
        ? readSpotifyCacheRecord(cacheUserId, cacheKey)
        : null;
      if (cachedRecord) {
      ...
      try {
        ...
        const artistsRaw = Array.isArray(result.data?.artists)
          ? result.data.artists
          : [];
        ...
        const album =
          result.data?.album && typeof result.data.album === "object"
            ? {
                id:
                  typeof result.data.album.id === "string"
                    ? result.data.album.id
                    : null,
                name:
                  typeof result.data.album.name === "string"
                    ? result.data.album.name
                    : null,
                totalTracks: Number.isFinite(result.data.album.total_tracks)
                  ? result.data.album.total_tracks
                  : null,
              }
            : null;
  ...
  if (
    req.method === "GET" &&
    url.pathname.startsWith("/api/albums/") &&
    url.pathname.endsWith("/tracks")
  ) {
    if (isDemoSession(cookies)) {
    ...
    (async () => {
      ...
      try {
        ...
        if (cacheUserId) {
          ...
          res.setHeader(
            "x-sp-cache",
            cachedRecord ? "refreshed" : "miss_stored",
          );
          res.setHeader("x-sp-cache-fetched-at", storedAt);
        }
  ...
  if (
    ...
    (async () => {
      ...
      try {
        if (!all) {
          const result = await fetchWithRefresh(token =>
            callTracks(token, { offset }),
          );
          if (!result.ok) {
            sendJson(res, result.status, {
              error: "spotify_playlist_tracks_failed",
              details: result.data,
              retryAfterSeconds: result.retryAfterSeconds,
            });
            return;
        ...
        while (nextOffset < total) {
          ...
          const page = await fetchWithRefresh(token =>
            callTracks(token, { offset: nextOffset }),
          );
          if (!page.ok) {
            if (canUseServerCache && cachedRecord) {
              res.setHeader("x-sp-cache", "hit_fallback");
  ...
  if (req.method === "GET") {
    ...
    if (shouldServeClientAppShell(url.pathname) && tryServeClientAppShell(res))
      return;
  }
...
server.listen(port, host, () => {
  ...
  console.log(
    `server listening on http://${displayHost}:${port} (bound to ${host})`,
  );
});
