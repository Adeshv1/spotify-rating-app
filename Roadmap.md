...
+ Playlists/UI: make the Playlists page mobile-friendly with a compact mobile header, tighter metadata/search spacing, and denser single-column playlist cards while preserving desktop layout (implemented 2026-03-25)
...
+ Header/UI: reorganize the signed-in responsive header into brand/actions, nav, and Export / Import rows with the storage alert grouped beside How to use and Log out (implemented 2026-03-25)
+ Header/UI: move the red storage alert beside `Export / Import` on desktop only while keeping the smaller-layout placement unchanged (implemented 2026-03-25)
+ Header/UI: center the mobile storage alert bubble and add an in-panel close `X` to the storage notice on all layouts (implemented 2026-03-25)
+ Header/UI: stack the mobile `Export / Import` control directly under the storage `!` button while keeping desktop grouping unchanged (implemented 2026-03-25)
+ Header/UI: move the mobile `Export / Import` trigger into the bottom center of the storage notice panel itself (implemented 2026-03-25)
+ Header/UI: rename the backup control to `Back up & Restore`, update the storage notice copy, and widen the trigger for the longer label (implemented 2026-03-25)
+ How to use/UI: keep the mobile dialog Close button pinned to the top right instead of dropping below the title block (implemented 2026-03-25)
+ Playlists/UI: compress mobile playlist cards into a much shorter side-by-side layout with smaller media, text, and actions while leaving desktop unchanged (implemented 2026-03-25)
+ Playlists/UI: replace the mobile-only hidden `In pool` affordance with a visible top-right pool status toggle that shows `In pool` with a green check or `Not in pool` with a red `X` (implemented 2026-03-25)
+ Playlists/UI: add a mobile-only `Remove from pool` button beside `Sync` so removal is available directly from the card actions (implemented 2026-03-25)
+ Playlists/UI: move the mobile `Play` button to the top left and make it slightly larger for easier tapping (implemented 2026-03-25)
+ Dialogs/UI: align shared modal Close buttons consistently so both help and confirm dialogs stay pinned to the top right on mobile (implemented 2026-03-25)
+ Playlist detail/UI: add artist avatars to the desktop tracks table using one cached image per primary artist instead of per-song image fetches (implemented 2026-03-25)
+ Header/UI: make the desktop `Back up & Restore` button inherit the same button styling as `How to use` while keeping the larger mobile notice trigger unchanged (implemented 2026-03-25)
+ Rank Songs: make binary-compare session counts and targets use the visible ranked list so stale hidden entries do not inflate counts or surface as unknown tracks (implemented 2026-03-25)
+ Playlist detail/UI: resolve artist avatars from the same track-to-artist-id flow used by dashboard artist cards instead of relying on row cache ids alone (implemented 2026-03-25)
+ Playlist detail/UI: replace the mobile track table with a dedicated card layout while keeping the desktop playlist-detail table intact (implemented 2026-03-25)
+ Playlist detail/UI: switch the card layout on before the desktop table clips at medium widths, instead of waiting for the narrow mobile breakpoint (implemented 2026-03-25)
+ Rank Songs/UI: add a mobile-only segmented tab row for Unranked, Rank, and Ranked, and auto-switch to Rank when starting a song from mobile (implemented 2026-03-25)
+ Rank Songs/UI: nudge the mobile Unranked action buttons inward so the trailing Play button clears the panel edge on narrow screens (implemented 2026-03-25)
+ Rank Songs/UI: rebalance the mobile Unranked action column so Rank and Play clear both left and right edges without manual offset clipping (implemented 2026-03-25)
+ Rank Songs/UI: disable the desktop-style sticky/internal-scroll Do not rate footer on mobile so the mobile panel flows naturally without the bottom gap artifact (implemented 2026-03-25)
+ Rank Songs/UI: auto-scroll slightly when Do not rate opens so the newly revealed content is immediately visible on mobile and inside scrollable panels (implemented 2026-03-25)
+ Rank Songs/UI: restore the mobile Do not rate bar to sticky-bottom behavior while keeping the newer mobile sizing and scroll cues in place (implemented 2026-03-25)
+ Rank Songs/UI: replace the mobile Unranked sticky Do not rate footer with a shared two-tab mobile panel so Unranked and Do not rate take turns using the full panel height (implemented 2026-03-25)
+ Rank Songs/UI: keep the mobile Do not rate tab on the same search-and-table shell as Unranked so the header, search bar, and Song/Action row stay consistent (implemented 2026-03-25)
+ Rank Songs/UI: hide the redundant mobile Unranked/Do not rate title above search, move the sub-tabs above search, add the Unranked count to that mobile tab, and widen the mobile action column so Restore fits cleanly (implemented 2026-03-25)
+ Rank Songs/UI: make the center Rank panel scroll vertically again so the Info section remains readable when opened (implemented 2026-03-25)
+ Rank Songs/UI: shrink the mobile compare cards and controls so the stacked duel fits more comfortably in the middle of the screen without changing desktop sizing (implemented 2026-03-25)
+ Rank Songs/UI: compact the mobile compare cards even further with tighter padding, smaller text, and shorter buttons so the stacked duel takes substantially less vertical space (implemented 2026-03-25)
+ Rank Songs/UI: demote and reposition the mobile compare-status line so “Comparing against rank …” sits closer to the top divider, uses smaller gray text, and draws less attention (implemented 2026-03-25)
+ Rank Songs/UI: replace the mobile Ranked table rows with compact artist-avatar cards that show song and album text above a Play/Up/Down/Reset action row while keeping the desktop table unchanged (implemented 2026-03-25)
